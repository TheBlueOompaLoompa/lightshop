...
func update_tracks():
    ...
    for track in project.tracks:
        ...
        track_scene.scale_px = scale_px
        track_scene.view_beats = view_beats
        ...
        track_scene.clip_saved.connect(func(clip):
            var new_clip = clip.duplicate(true)
            new_clip.type = track.type
            if new_clip is EffectClip:
                for effect in new_clip.effects:
                    effect.dupe()
            project.saved_clips.append(new_clip)
...
func _on_open_project(project_name: String) -> void:
    ...
    track_visibility.project = project
    clips_pool.project = project
    reset_ui()
...
func _on_clip_button_pressed():
    var clip_scene = CLIP.instantiate()
...
func _on_clips_add(c: Clip) -> void:
    var clip_scene = CLIP.instantiate()
    var clip = c.duplicate(true)
    clip.selected = false
    clip.start = 0
    clip.end = 1
